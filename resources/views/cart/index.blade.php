    ...
    @else
        ...
        <div class="mt-5 space-y-3">
            @foreach($quote['lines'] as $line)
                <div class="flex items-center justify-between gap-3 rounded-2xl bg-surface-container-lowest border border-slate-100 p-4 shadow-xs">
                    <div class="min-w-0">
                    ...
                    <div class="font-bold text-on-surface shrink-0">{{ number_format($line['line_total'], 2) }} <span class="ils">₪</span></div>
                </div>
            @endforeach
        </div>
        <aside class="mt-6 rounded-2xl bg-surface-container-lowest border border-slate-100 p-5 shadow-xs">
            <div class="flex justify-between text-sm text-on-surface-variant"><span>المجموع</span><span>{{ number_format($quote['subtotal'], 2) }} <span class="ils">₪</span></span></div>
            @if($quote['discount_percent'])
                <div class="mt-2 flex justify-between text-sm text-secondary font-semibold"><span>خصم العضوية {{ $quote['discount_percent'] }}%</span><span>- {{ number_format($quote['discount_amount'], 2) }} <span class="ils">₪</span></span></div>
            @endif
            ...
            <div class="mt-4 flex justify-between text-lg font-bold text-on-surface"><span>النهائي</span><span>{{ number_format($quote['total'], 2) }} <span class="ils">₪</span></span></div>
            <p class="mt-3 text-sm text-primary font-medium">ستحصل تقريباً على {{ $quote['points'] }} نقطة بعد التسليم.</p>
